...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        switch (joinOp){
            ...
            case LESS_THAN:
            case NOT_EQUALS:
                card = (int)((double)card1 * (double)card2 * (1.0 / 3.0));
                break;
        }
        // some code goes here
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        for(int i = 1; i <= stats.size(); ++i){
            Set<Set<LogicalJoinNode>> subplans = enumerateSubsets(this.joins, i);
            for (Set<LogicalJoinNode> subplan : subplans){
                best = null;
                for(LogicalJoinNode r : subplan) {
                    double cur_best_cost = best == null ? Double.MAX_VALUE : best.cost;
                    if(subplan.contains(r)) {
                        cur = computeCostAndCardOfSubplan(stats, filterSelectivities, r, subplan, cur_best_cost, pc);
                        if(cur != null) best = cur;
                    }
                }
                if(best != null)
                    pc.addPlan(subplan, best.cost, best.card, best.plan);
            }
        }
