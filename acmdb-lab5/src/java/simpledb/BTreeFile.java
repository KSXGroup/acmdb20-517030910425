...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		BTreePage pageCurrent;
		if(pid.pgcateg() == BTreePageId.LEAF){
			pageCurrent = (BTreePage) this.getPage(tid, dirtypages, pid, perm);
			return (BTreeLeafPage) pageCurrent;
		}
		else{
			BTreeInternalPage pageToSearch = (BTreeInternalPage) this.getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
			if(f == null){
				return findLeafPage(tid, dirtypages, pageToSearch.getChildId(0), perm, f);
			}else {
				BTreeEntry e = null;
				Iterator<BTreeEntry> entryIterator = pageToSearch.iterator();
				...
				if(e != null) return findLeafPage(tid, dirtypages, e.getRightChild(), perm, f);
				else return null;
			}
		}
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        dirtypages.putIfAbsent(emptyLeafPage.getId(), emptyLeafPage);
        int numTuple = page.getNumTuples();
		int numTupleToMove = numTuple >> 1;
        BTreePageId oldRightSiblingId = page.getRightSiblingId();
        Iterator<Tuple> tupleIterator = page.reverseIterator();
        for(int i = 0; i < numTupleToMove; ++i){
        	Tuple t = tupleIterator.next();
			page.deleteTuple(t);
        	emptyLeafPage.insertTuple(t);
		}
        emptyLeafPage.setRightSiblingId(oldRightSiblingId);
        emptyLeafPage.setLeftSiblingId(page.getId());
        page.setRightSiblingId(emptyLeafPage.getId());
        if(oldRightSiblingId != null){
        	BTreeLeafPage rightSibling = (BTreeLeafPage)getPage(tid, dirtypages, oldRightSiblingId, Permissions.READ_WRITE);
        	rightSibling.setLeftSiblingId(emptyLeafPage.getId());
		}
		Tuple firstInNewPage = emptyLeafPage.getTuple(0);
        Field middleKey = firstInNewPage.getField(keyField());
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), middleKey);
        parentPage.insertEntry(new BTreeEntry(middleKey, page.getId(), emptyLeafPage.getId()));
		updateParentPointer(tid, dirtypages, parentPage.getId(), emptyLeafPage.getId());
        if(field.compare(Op.GREATER_THAN_OR_EQ, middleKey)) return emptyLeafPage;
        else return page;
	}
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
			BTreeInternalPage page, Field field)
					throws DbException, IOException, TransactionAbortedException {
		BTreeInternalPage emptyInternalPage = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
		dirtypages.putIfAbsent(emptyInternalPage.getId(), emptyInternalPage);
		int numEntries = page.getNumEntries();
		int numEntriesToRemove = (numEntries >> 1);
		if((numEntries & 1) == 0){
			Field tmpMidKey = page.getKey(numEntriesToRemove);
			if(field.compare(Op.GREATER_THAN, tmpMidKey))
				numEntriesToRemove -= 1;
		}
		...
		for(int i = 0; i < numEntriesToRemove; ++i){
			...
			emptyInternalPage.insertEntry(curEntry);
		}
		BTreeEntry tmpE = it.next();
		Field middleKey = tmpE.getKey();
		page.deleteKeyAndRightChild(tmpE);
		updateParentPointers(tid, dirtypages, emptyInternalPage);
		...
		parentPage.insertEntry(new BTreeEntry(middleKey, page.getId(), emptyInternalPage.getId()));
		updateParentPointer(tid, dirtypages, parentPage.getId(), emptyInternalPage.getId());
		if(field.compare(Op.LESS_THAN, middleKey))
			return page;
		else
			return emptyInternalPage;
	}
	...
	Page getPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm)
			throws DbException, TransactionAbortedException {
		if(dirtypages.containsKey(pid) && dirtypages.get(pid) != null) {
	...
	private void handleMinOccupancyLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
		...
		if(leftSiblingId != null) {
			...
			if(leftSibling.getNumEmptySlots() >= maxEmptySlots) {
				...
				//System.out.println("steal: " + tid.hashCode());
...
//		else
//			s = "Right";
//		System.out.println("STEAL LEAF " + s);
		int numToMove = (sibling.getNumTuples() - page.getNumTuples()) >> 1;
		if(numToMove < 0)
			throw new DbException("No enough tuple on the sibling");
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// some code goes here
		//System.out.println("STEAL INTERNAL LEFT " + tid.hashCode());
		int numRightPageEntry = page.getNumEntries();
		int numLeftPageEntry = leftSibling.getNumEntries();
		if(numLeftPageEntry < numRightPageEntry) throw new DbException("Invalid Stealing!");
		int numToMove = ((numLeftPageEntry - numRightPageEntry) >> 1) - 1;
		Iterator<BTreeEntry> leftSiblingIterator = leftSibling.reverseIterator();
		BTreeEntry e = null;
		BTreePageId tmpLeft = leftSibling.reverseIterator().next().getRightChild();
		BTreePageId tmpRight = page.iterator().next().getLeftChild();
		Field keyPushedDown = parentEntry.getKey();
		BTreeEntry entryToPushDown = new BTreeEntry(keyPushedDown, tmpLeft, tmpRight);
		page.insertEntry(entryToPushDown);
		for(int i = 0; i < numToMove; ++i){
			e = leftSiblingIterator.next();
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println(" STEAL INTERNAL RIGHT " + tid.hashCode());
		int numRightPageEntry = rightSibling.getNumEntries();
		int numLeftPageEntry = page.getNumEntries();
		if(numLeftPageEntry > numRightPageEntry) throw new DbException("Invalid Stealing!");
		int numToMove = ((numRightPageEntry - numLeftPageEntry) >> 1) - 1;
		Iterator<BTreeEntry> rightSiblingIterator = rightSibling.iterator();
		BTreeEntry e = null;
		BTreePageId tmpRight = rightSibling.iterator().next().getLeftChild();
		BTreePageId tmpLeft = page.reverseIterator().next().getRightChild();
		Field keyPushedDown = parentEntry.getKey();
		BTreeEntry entryToPushDown = new BTreeEntry(keyPushedDown, tmpLeft, tmpRight);
		page.insertEntry(entryToPushDown);
		for(int i = 0; i < numToMove; ++i){
			e = rightSiblingIterator.next();
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		//System.out.println(" MERGE LEAF " + tid.hashCode());
		...
		if(newRightSiblingId != null){
			BTreeLeafPage rightSiblingOfRightSibling = (BTreeLeafPage)
					getPage(tid, dirtypages, newRightSiblingId, Permissions.READ_WRITE);
			rightSiblingOfRightSibling.setLeftSiblingId(leftPage.getId());
		}
		while(rightPageIterator.hasNext()){
			t = rightPageIterator.next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		//System.out.println(" MERGE INTERNAL " + tid.hashCode());
		Iterator<BTreeEntry> rightPageIterator = rightPage.iterator();
		BTreePageId tmpRight = rightPage.iterator().next().getLeftChild();
		BTreePageId tmpLeft = leftPage.reverseIterator().next().getRightChild();
		Field tmpKey = parentEntry.getKey();
		leftPage.insertEntry(new BTreeEntry(tmpKey, tmpLeft, tmpRight));
		BTreeEntry e;
		while(rightPageIterator.hasNext()){
			e = rightPageIterator.next();
	...
	public ArrayList<Page> deleteTuple(TransactionId tid, Tuple t)
		...
		page.deleteTuple(t);
		//System.out.println("Delete " + t.getField(0));
		//System.out.println(tid.hashCode() + " Before delete " + dirtypages.size());
		...
		//System.out.println(tid.hashCode() + " After delete " + dirtypages.size());
		ArrayList<Page> dirtyPagesArr = new ArrayList<Page>();
		dirtyPagesArr.addAll(dirtypages.values());
		return dirtyPagesArr;
	}
